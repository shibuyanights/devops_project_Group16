...
class Dog(Game):
    ...
    def _is_valid_finish_move(self, marble: Marble, move: int) -> bool:
        # Assuming finish area is range(92, 96)
        if marble.pos >= 92:
            next_pos = marble.pos + move
            return 92 <= next_pos < 96  # Within finish range
        return True  # Normal moves are always valid

    def _is_finish_overtaking(self, marble: Marble, move: int) -> bool:
